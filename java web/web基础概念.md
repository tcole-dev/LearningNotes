...
<font color='red'>现在基本都不会直接写`Servlet`，这是JSP时代才会做的，现代web一般直接使用`SpringBoot`等框架进行开发。</font>
...
在Java Web开发中，过滤器（Filter）是一种用于对请求（Request）和响应（Response）进行预处理和后处理的组件。它们主要用于拦截客户端发送到服务器的请求，以及服务器发送给客户端的响应，以执行一些诸如认证、日志记录、字符编码转换等通用任务。过滤器是一种强大的工具，可以提高代码的可重用性和模块化。

过滤器有点类似于`Spring`中的`AOP`面向切面编程，但AOP侧重于业务代码中的重复部分，将其封装为切面，自动插入；而过滤器则侧重于http请求的校验等，如可以将身份校验封装为一个过滤器，每次http请求必须通过过滤器才能转发到`Controller`。
...
public class SimpleJdbc {
    public static void main(String[] args) {
        ...
        // 1. 建立连接并创建执行对象 ( try-resource 自动关闭资源)
